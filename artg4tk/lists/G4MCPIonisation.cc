...
G4MCPIonisation::G4MCPIonisation(const G4String& name)
...
G4bool G4MCPIonisation::IsApplicable(const G4ParticleDefinition& p)
{
...
G4double G4MCPIonisation::MinPrimaryEnergy(const G4ParticleDefinition*,
					  const G4Material*,
					  G4double cut)
{
...
void G4MCPIonisation::InitialiseEnergyLossProcess(const G4ParticleDefinition* part,
                                                 const G4ParticleDefinition* bpart)
{
  if(!isInitialised) {
    ...
    // high energy model
    if(ehigh < emax) {
      if (!EmModel(2)) { SetEmModel(new G4BetheBlochModel_MCP()); }
...
void G4MCPIonisation::PrintInfo()
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void G4MCPIonisation::ProcessDescription(std::ostream& out) const
{
  out << "  MCP ionisation";
