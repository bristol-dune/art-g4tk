...
//
//
// File name:     G4BetheBlochModel_MCP
//
...
#include "G4BetheBlochModel_MCP.hh"
...
G4double G4BetheBlochModel_MCP::xgi[]={ 0.0199, 0.1017, 0.2372, 0.4083, 0.5917,
                                      0.7628, 0.8983, 0.9801 };
                                      
G4double G4BetheBlochModel_MCP::wgi[]={ 0.0506, 0.1112, 0.1569, 0.1813, 0.1813,
                                      0.1569, 0.1112, 0.0506 };

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4BetheBlochModel_MCP::G4BetheBlochModel_MCP(const G4ParticleDefinition* p,
                                         const G4String& nam)
  : G4VEmModel(nam),
...
G4double G4BetheBlochModel_MCP::MinEnergyCut(const G4ParticleDefinition*,
...
G4double G4BetheBlochModel_MCP::MaxSecondaryEnergy(const G4ParticleDefinition*,
                                                 G4double kinEnergy) 
{
...
void G4BetheBlochModel_MCP::SetParticle(const G4ParticleDefinition* p)
{
  if(nullptr == particle) {
    particle = p;
...
void G4BetheBlochModel_MCP::Initialise(const G4ParticleDefinition* p,
                                     const G4DataVector&)
{
  SetParticle(p);
...
G4double G4BetheBlochModel_MCP::ComputeCrossSectionPerElectron(
                                                 ...
                                                 G4double maxKinEnergy)
{
  ...
  if(cutEnergy < maxEnergy) {
    ...
    cross *= chargeSquare * CLHEP::twopi_mc2_rcl2/beta2;
  }
...
G4double G4BetheBlochModel_MCP::ComputeCrossSectionPerAtom(
							 const G4ParticleDefinition* p,
							 G4double kineticEnergy,
							 G4double Z, G4double,
							 G4double cutEnergy,
...
G4double G4BetheBlochModel_MCP::CrossSectionPerVolume(
...
G4double G4BetheBlochModel_MCP::ComputeDEDXPerVolume(const G4Material* material,
						   const G4ParticleDefinition* p,
						   G4double kineticEnergy,
						   G4double cut)
{
  ...
  dedx *= chargeSquare * CLHEP::twopi_mc2_rcl2*eDensity/beta2;
...
void G4BetheBlochModel_MCP::SampleSecondaries(
					    ...
					    G4double maxEnergy)
{
  ...
  do {
    ...
    if(f > grej) {
      G4cout << "G4BetheBlochModel_MCP::SampleSecondary Warning! "
	     << "Majorant " << grej << " < "
	     << f << " for edelta= " << tkin
